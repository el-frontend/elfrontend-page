  ...
  return (
    <nav className="container" style={{
      ...
      <div style={{display:'flex', gap:24, color:'var(--dim)', fontSize:11, letterSpacing:'0.1em', flexWrap:'wrap', justifyContent:'center'}}
        ...
        {Object.entries(t.nav).map(([k,n],i)=>{
          const id = '#'+k;
          return (
            <a key={k} href={id}
              onClick={()=>playClick()}
              style={{color: i===0?'var(--hot-yellow)':'var(--dim)', position:'relative', padding:'4px 0'}}
              onMouseEnter={e=>{ e.currentTarget.style.color='var(--ink)'; playHover(); }}
              onMouseLeave={e=>{ e.currentTarget.style.color = i===0?'var(--hot-yellow)':'var(--dim)'; }}
            >
              {i===0 && <span style={{position:'absolute', left:-16, top:'50%', transform:'translateY(-50%)'}}><PxArrow scale={2} color="var(--hot-yellow)"/></span>}
              {n}
            </a>
          );
        })}
                ...
                {g.skills.map(s => {
                  ...
                  return (
                    <div key={s.name}
                      ...
                      <div style={{minWidth:120, fontFamily:'var(--font-pixel-body)', fontSize:20, color:'var(--ink)'}}>{s.name}</div>
    ...
    <section id="blog" style={{padding:'72px 32px', background:'var(--paper-2)', borderTop:'4px solid var(--dim-2)', borderBottom:'4px solid var(--dim-2)'}}>
      <div className="container">
        <div className="sec-head">
          ...
          <a className="px-link pixel-text" href="https://elfrontend.com/blog" target="_blank" rel="noreferrer" style={{fontSize:10, letterSpacing:'0.2em'}}>
            {t.blogCta} <PxArrow scale={2}/>
          </a>
        </div>
        <div style={{display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:18}}>
          {BLOG.map((b,i)=>(
            <a key={b.id} href={"https://elfrontend.com/blog/"+b.slug} target="_blank" rel="noreferrer"
              onClick={()=>playCoin()}
              className="pcard"
